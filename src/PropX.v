...
Section machine.
  ...
  Section specs.
      ...
      repeat match goal with
               | [ H : _ = ?E |- _ ] => 
                 match E with
                   | Some _ => fail 1
                   | _ =>
                     (generalize (inj_pair2 _ _ _ _ _ H) || injection H);
                     clear H; intros; repeat match goal with
                                               | [ H : ?X = ?Y |- _ ] => subst X || subst Y
                                             end
                 end
             end.

    Lemma cut_admissibility' : forall GG (P : propX GG) (Heq : GG = nil) PG Q, seq PG Q
      -> forall G, incl PG (match Heq in _ = G return propX G with
                              | refl_equal => P
                            end :: G)
        -> seq G (match Heq in _ = G return propX G with
                    | refl_equal => P
                  end)
        -> seq G Q.
      induction P; (intro Heq; intros; generalize dependent Heq; intro Heq; generalize Heq; subst;
        intro Heq; rewrite (UIP_refl _ _ Heq); clear Heq; intros;
          repeat match goal with
                   | [ H : _ |- _ ] => specialize (H (refl_equal _))
                     || (generalize (fun x => H x (refl_equal _)); clear H; intro H)
                 end; simpl in *;
          match goal with
    ...
    Theorem cut_admissibility : forall P G Q,
      ...
      -> seq (P :: G) Q
      -> seq G Q.
      intros; eapply (cut_admissibility' _ (refl_equal _)); eauto.
    Qed.
