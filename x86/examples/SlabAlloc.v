...
Module SlabAlloc.
  ...
  Definition specExtendHeap : state -> PropX pc state :=
    ...
    ![ !{allocated st#RBX (wordToNat st#RCX * wordToNat st#RDX)} * !{heap (wordToNat st#RDX) st#RAX n} * ![fr] *
       [< noneNull st#RBX (wordToNat st#RCX * wordToNat st#RDX) >] *
       [< st#RBX <> NULL >] ] st
  ...
  Proof.
    ...
    generalize (!{allocated (p ^+ wone W64) n'}). change (wone W64) with (natToWord 64 1). word_simpl_in_star.
  ...
  Lemma minus1_times_n_eq_minus_n : forall sz (a : word sz) c,
    (wordToN (a ^- ##1)%nat * c = wordToN a * c - c)%N.
  Proof.
  Admitted.
    ...
    bfunction "extendHeap" [specExtendHeap] {
      ...
      [st ~> Ex fr, Ex fl,
        ![ [< (8 <= wordToNat st#_width)%nat >] * 
           [< noneNull st#_buf (wordToNat st#_count * wordToNat st#_width) >] *
...
*)
  ...
  Hint Extern 1 (noneNull _ _) => (eapply noneNull_move_noneNull; [ match goal with 
                                                                      | [ H : _ |- _ ] => eapply H
                                                                    end | eauto ]).
  Hint Resolve times0_eq_0 plus0_eq_b.
  Hint Resolve Neq_Nle minus1_times_n_eq_minus_n. 
  Hint Resolve eight_lt_prod.
  Hint Resolve noneNull_not_null noneNull_move_noneNull.
  ...
  Theorem slab_allocOk : moduleOk slab_alloc.
    ...
    sep; eauto; simpl; auto. word_simpl_in_star. SearchAbout noneNull.
    eapply noneNull_move_noneNull. eassumption.
    
    Focus 2.    SearchAbout noneNull.
    Hint Resolve noneNull_not_null_start. eauto.
  Qed.
